export const sidebar = {
  '/javascript': [
    {
      text: 'JavaScript',
      items: [
        { text: '深度解析 JavaScript 中 var、let、const 的区别', link: '/javascript/1' },
        { text: 'JavaScript/ECMAScript 中的数据类型全解析', link: '/javascript/2' },
        { text: 'JavaScript 中的内置对象：探索核心语言的强大工具', link: '/javascript/3' },
        { text: 'JavaScript 中的 JSON：数据交换的强大工具', link: '/javascript/4' },
        { text: 'JavaScript 中的 Map 和 WeakMap 详解', link: '/javascript/1' },
        { text: 'JavaScript 中的 Set 和 WeakSet 详解', link: '/javascript/2' },
        { text: 'JavaScript 中的 TypedArray 详解', link: '/javascript/7' },
        { text: 'JavaScript 中 ==、=== 和 Object.is() 有什么区别？', link: '/javascript/8' },
        { text: 'JavaScript 中箭头函数和普通函数的区别', link: '/javascript/9' },
        { text: 'JavaScript 中的闭包详解', link: '/javascript/10' },
        { text: '手写实现 call、apply 和 bind 函数', link: '/javascript/11' },
      ],
    },
  ],
  '/browser': [
    {
      text: '浏览器原理',
      items: [
        { text: '浏览器中的事件循环原理', link: '/browser/1' },
        { text: '浏览器中页面渲染原理', link: '/browser/2' },
      ],
    },
  ],
  '/blog/': [
    {
      text: '实战',
      items: [
        { text: 'TypeScript 中 type 和 interface 有什么区别', link: '/blog/9' },
        { text: 'Vue3 的响应式原理简介', link: '/blog/7' },
        { text: '手写实现数组实例方法', link: '/blog/4' },
        { text: '使用 Docker 一键部署带 Let\'s Encrypt SSL 证书的 Nginx', link: '/blog/3' },
        { text: '在 Vue 中实现音频可视化', link: '/blog/2' },
        { text: 'JavaScript 脚本的加载时机', link: '/blog/1' },
      ],
    },
  ],
  '/project/': [
    {
      text: '项目',
      items: [
      ],
    },
  ],
}
